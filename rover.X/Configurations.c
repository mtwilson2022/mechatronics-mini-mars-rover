...
void configurePins() {
    ...
    ANSA = 0;
//    _ANSB12 = 1;        //ROS: line follow left
//    _ANSB13 = 1;        //ROS: line follow center
//    _ANSB14 = 1;        //ROS: line follow right
//    _ANSB15 = 1;        //ROS: ball color detection
    TRISA = 1;          // initially set all pins to input, set outputs after
    TRISB = 1;
    _TRISB0 = 0;        //motor one PWM (left motor) (pin set to output)
    _TRISB1 = 0;        //motor two PWM (right motor))
    _TRISA0 = 0;        //motor one dir 
    _TRISA1 = 0;        //motor two dir
    _TRISB8 = 0;        //debug LED pin 
...
void configAD() {
    ...
    _CSCNA = 1;     // AD1CON2<10> -- Does not scan inputs specified in AD1CSSx
                    // registers
    _SMPI = 1;    // AD1CON2<6:2> -- Results sent to buffer after n conversion
...
//    _CH0SA = 0b01101;    // AD1CHS<4:0> -- Use AN2 (pin 4) as positive input
